...
int distance;             // step size of the glissandos
...
void mainloopalgorithms(void) { 
   ...
   while (synth.getNoteCount() > 0) {
      ...
      if (message.p2() != 0) {
         ...
         distance = lastnotes[0] - lastnotes[1];
         duration = lasttimes[0] - lasttimes[1];
         channel = 0x0f & message.p0();
         if (distance != 0) {
            playgliss(message.p1(), message.p2(), channel,  duration, distance);
         }
      }
   }
}
...
void playgliss(int basenote, int loudness, int channel, int duration, 
   ...
   tn.shortValue(14) = distance;

   tn.setFunction(EnhanceFunction);
   tn.setChannel(channel);
   tn.setKeyno(basenote + distance);
