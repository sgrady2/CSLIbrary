...
RadioBaton::RadioBaton(void) : MidiIO() {
   ...
   writingCalib = 0;
   calibFilename = NULL;
   calibFilename = new char[100];
   strcpy(calibFilename, "calib.def");
...
//////////////////////////////
//
// RadioBaton::centerStick1 --
//

void RadioBaton::centerStick1(void) {
   sendMessage(SET_CENTER_STICK1);
}
...
//////////////////////////////
//
// RadioBaton::centerStick2 --
//

void RadioBaton::centerStick2(void) {
   sendMessage(SET_CENTER_STICK2);
}
...
//////////////////////////////
//
// RadioBaton::checkCalibration -- must have monotonically increasing
//     values in calib array locations 10-20, and values must be less 
//     than 2048.
//

int RadioBaton::checkCalibration(void) {
   int value = calib[10];
   for (int i = 11; i<21; i++) {
      if (calib[i] <= value || calib[i] >= 2048) {
         return 0;
      }
      value = calib[i];
   }
...
//////////////////////////////
//
// RadioBaton::decreaseXYSensitivity --
//
 
void RadioBaton::decreaseXYSensitivity(void) {
   sendMessage(DEC_XY_SENSITIVITY);
}
...
//////////////////////////////
//
// RadioBaton::decreaseZSensitivity --
//

void RadioBaton::decreaseZSensitivity(void) {
   sendMessage(DEC_Z_SENSITIVITY);
}
...
//////////////////////////////
//
// RadioBaton::increaseXYSensitivity --
//
 
void RadioBaton::increaseXYSensitivity(void) {
   sendMessage(INC_XY_SENSITIVITY);
}
...
//////////////////////////////
//
// RadioBaton::increaseZSensitivity --
//

void RadioBaton::increaseZSensitivity(void) {
   sendMessage(INC_Z_SENSITIVITY);
}
...
//////////////////////////////
//
// RadioBaton::readCalibrationFile -- send calibration data from
//    radio baton to the specified file. If file couldn't be
//    found, return 0; otherwise, returns 1.
//

int RadioBaton::readCalibrationFile(const char* aFilename) {
   fstream input(aFilename, ios::in | ios::nocreate);
   if (!input) {
      return 0;
   }
   char tempString[256];
   
   input >> tempString;     // hit1
   input >> calib[0];
   input >> tempString;     // set1
   input >> calib[1];
   input >> tempString;     // hit2
   input >> calib[2];
   input >> tempString;     // set2
   input >> calib[3];
   input >> tempString;     // zero1x
   input >> calib[4];
   input >> tempString;     // zero1y
   input >> calib[5];
   input >> tempString;     // zero2x
   input >> calib[6];       
   input >> tempString;     // zero2y
   input >> calib[7];
   input >> tempString;     // range
   input >> calib[8];       
   input >> tempString;     // restpoint
   input >> calib[9];
   for (int i=10; i<21; i++) {
      input >> calib[i];
   }
   input >> tempString;      // zcalib
   input >> calib[21];
 
   // send calibration data to the radio drum only if calibration file is 
   // read completely.  Check by comparing last string which should be "zcalib".
   for (int j=0; j<6; j++) {
      if (tempString[j] == '\0') break;
      tempString[j] = (char)tolower(tempString[j]);
   }
   if (checkCalibration() == 0) {
      cerr << "Calibration file is corrupt; file " << aFilename
           << " was not sent." << endl;
      return 0;
   }
   if (strcmp("zcalib", tempString) == 0 ) {
      // now tell the radio drum what the calibration is:
      sendCalibration();      

      return 1;       // successful reading of calibration file.
   } else {
      return 0;       // unsuccessful reading of calibration file.
   }
}
...
//////////////////////////////
//
// RadioBaton::requestCalibration -- Send a request for calibration data 
//    from the radio drum.  It will be received later from the 
//    radio drum and stored in the calib array.
//

void RadioBaton::requestCalibration(void) {
   stoprunningstatus(); 
   rawsend(BAT_MIDI_COMMAND, REPORT_CALIB, 0);
}
...
//////////////////////////////
//
// RadioBaton::requestBufData -- Gets all 16 values of buf data
//    from the radio drum.
//

void RadioBaton::requestBufData(void) {
   // send request for buffer data
   for (int i=0; i<16; i++) {
      stoprunningstatus();
      rawsend(BAT_MIDI_COMMAND, REPORT_BUF, i);
   }
   // the processIncomingMessages function will detect the results
   // of this buff data request and fill the buff array.
}
...
//////////////////////////////
//
// RadioBaton::setStickLevels --
//

void RadioBaton::setStickLevels(void) {
   sendMessage(SET_STICK_LEVELS);
}
...
//////////////////////////////
//
// RadioBaton::testBaton -- checks to see if radio drum is connected
//

int RadioBaton::testBaton(void) {
   int reportStatus = positionReportingOff();
   stoprunningstatus();
   rawsend(BAT_MIDI_COMMAND, BAT_SEND, TEST_BATON);

   #ifdef VISUAL
      Sleep(20);           // 20 milliseconds
   #else
      usleep(20000);       // 20000 microseconds = 20 milliseconds
   #endif

   // search for OK receive message
   if (MidiInput::getCount() == 0) {  
      // nothing came back via MIDI input
      setReportStatus(reportStatus);
      return 0;
   } else {
      // something came in MIDI input, check for baton alive message
      for (int i=0; i<MidiInput::getCount(); i++) {
         if ( (*this)[i].command() == BAT_MIDI_COMMAND &&
              (*this)[i].p1() == BAT_SEND &&
              (*this)[i].p2() == 0 ) {
                  setReportStatus(reportStatus);
                  return 1;
...
//////////////////////////////
//
// RadioBaton::writeCalibrationFile -- request the calibration
//     data from the radio drum and stores the filename to which
//     the data is to be written to when it has been received from 
//     the radio drum.
//

int RadioBaton::writeCalibrationFile(const char* aFilename) {
   pushReportingStatus();
   positionReportingOff();

   // ask for calibration data from the radio drum   
   requestCalibration();
   writingCalib = 1;

   // store the filename for writeCalibrationData() function
   // which will be called when the data is received via MIDI input
   // from the radio drum.
   if (calibFilename != NULL) {
      delete [] calibFilename;
   }
   calibFilename = new char[strlen(aFilename) + 1];
   strcpy(calibFilename, aFilename);
...
void RadioBaton::interpretCommand(MidiMessage aMessage) {
   ...
   if (aMessage.command() == BAT_MIDI_COMMAND) {
      switch (aMessage.p1()) {
         ...
         case BAT_SEND:                     // baton alive message et al.
            // ignore this command it is handled in the testBaton function
            break;   
         default:
            if (errorQ) {
   ...
   // take care of calibration data
   // nibbles are replaced as they come in from the radio drum
   else if (aMessage.command() == 0xa1) {
      // if p1 is not in the range from 0 to 21 then ignore the error:
      if (aMessage.p1() > 21) return;
      value = aMessage.p2();
      value &= 0x000f;                         // guarentee no high bits
      calib[aMessage.p1()] &= 0x0fff;          // blank out old nibble
      calib[aMessage.p1()] |= value << 12;
   } else if (aMessage.command() == 0xa2) {
      // if p1 is not in the range from 0 to 21 then ignore the error:
      if (aMessage.p1() > 21) return;
      value = aMessage.p2();
      value &= 0x000f;                         // guarentee no high bits
      calib[aMessage.p1()] &= 0xf0ff;          // blank out old nibble
      calib[aMessage.p1()] |= value << 8;
   } else if (aMessage.command() == 0xa3) {
      // if p1 is not in the range from 0 to 21 then ignore the error:
      if (aMessage.p1() > 21) return;
      value = aMessage.p2();
      value &= 0x000f;                         // guarentee no high bits
      calib[aMessage.p1()] &= 0xff0f;          // blank out old nibble
      calib[aMessage.p1()] |= value << 4;
   } else if (aMessage.command() == 0xa4) {
      // if p1 is not in the range from 0 to 21 then ignore the error:
      if (aMessage.p1() > 21) return;
      value = aMessage.p2();
      value &= 0x000f;                         // guarentee no high bits
      calib[aMessage.p1()] &= 0xfff0;          // blank out old nibble
      calib[aMessage.p1()] |= value;
 
      // write the calib file if last nibble of last element received:
      if (aMessage.p1() == 21 && writingCalib) {
         writingCalib = 0;

         // finished receiving calib data from radio drum.  The data
         // is sent only when a user request to write data to a file.
  	      // so write the file now.
         if (checkCalibration() != 0) {
            writeCalibrationData();
            cerr << "Calibration data written to file: "
                 << calibFilename << endl;
         } else {
            cerr << "Corrupt calibration.  Did not write to file." << endl;
         }
         // set the position reporting to what it was before the
         // calibration was requested from the radio drum
         popReportingStatus();
      }
   }
...
//////////////////////////////
//
// RadioBaton::sendCalibration -- Sends the 
//    calibration data stored in object to the
//    radio drum.  size of calib array is 22.
//

int RadioBaton::sendCalibration(void) {
   int status = checkCalibration();
   if (status != 0) {
      for (int j=0; j<22; j++) {
         MidiOutput::rawsend(0xa1, j, (calib[j] >> 12) & 0x000f);
         MidiOutput::rawsend(0xa2, j, (calib[j] >> 8)  & 0x000f);
         MidiOutput::rawsend(0xa3, j, (calib[j] >> 4)  & 0x000f);
         MidiOutput::rawsend(0xa4, j, (calib[j])       & 0x000f);
      }
   }
   return status;
}
...
//////////////////////////////
//
// RadioBaton::writeCalibrationData -- writes calibration data
//	from calib array.  This function is called after
//	a successful transmission of calib data array from
//	the radio drum  or you can just write the current 
//	contents of the array as it is stored in the computer
//	(which may be different from the current state in the
//	radio drum.
//

void RadioBaton::writeCalibrationData(void) {
   // successful receipt of calibration, so write it to a file:
   fstream output(calibFilename, ios::out);
   if (!output.is_open()) {
      cerr << "Error: could not open " << calibFilename 
           << " to write calibration data." << endl;
      // not a fatal error, however
      return;
   }

   output << "hit1      " << calib[0] << endl;
   output << "set1      " << calib[1] << endl;
   output << "hit2      " << calib[2] << endl;
   output << "set2      " << calib[3] << endl;
   output << "zero1x    " << calib[4] << endl;
   output << "zero1y    " << calib[5] << endl;
   output << "zero2x    " << calib[6] << endl;
   output << "zero2y    " << calib[7] << endl;
   output << "range     " << calib[8] << endl;
   output << "restpoint " << calib[9] << endl;
   for (int i=10; i<21; i++) {   // 22 elements in calib array
       output << calib[i] << endl;
   }
   output << "zcalib    " << calib[21] << endl;
...
// md5sum:        d41bb535ac0176953765441b1bed8150  RadioBaton.cpp
