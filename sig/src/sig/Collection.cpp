...
// Filename:      ...sig/code/base/Collection/Collection.cpp
// Web Address:   http://www-ccrma.stanford.edu/~craig/improv/src/Collection.cpp
// Syntax:        C++ 
//
...
void Collection<type>::grow(long growamt) {
   ...
   if (maxSize != 0 && getAllocSize() > maxSize) {
      cerr << "Error: Maximum size allowed for array exceeded." << endl;
      exit(1);
   }
...
void Collection<type>::setAllocSize(long aSize) {
   if (aSize < getSize()) {
      cerr << "Error: cannot set allocated size smaller than actual size." 
           << endl;
      exit(1);
   }
...
// md5sum:        95056e8ce909909b78c8fa739bb190cb  Collection.cpp
