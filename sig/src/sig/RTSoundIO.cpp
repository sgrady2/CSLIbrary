 ...
 * under DirectSound.  One method is to always write new buffers
 * of data behind the read pointer.  This method is very safe but
 * inherently produces a delay equivalent to the entire secondary
 * buffer (plus any other delays that Microsloth provides).  The
 * other method is to write new buffers in front of the read
 * pointer.  Even in this scenario, Microsloth requires that you
 * leave about 15 ms of buffer between the read and write pointers.
 * I've tried both methods and neither results in performance that
 * can compare to either Linux or SGI sound output.  In order to
 * minimize latency, however, I'll go with the second method above
 * and leave the first method commented out below.
 */
...
RTSoundIO :: RTSoundIO(MY_FLOAT srate, int channels)
{
   ...
   * read pointer (second method above).  However, if you use the
   * first method above, this variable will be more critical and
   * you'll probably want to make it smaller.
   */
  int nBufs = 12;

  /* Determine the maximum distance that the write pointer is
   * allowed to lead safePos.  The size of this zone is fairly
   * critical to the behavior of this scheme.
   */
  if (RT_BUFFER_SIZE < 200)
    zoneSize = 3*RT_BUFFER_SIZE*sizeof(short);  // bytes
  else if (RT_BUFFER_SIZE < 400)
    zoneSize = 2*RT_BUFFER_SIZE*sizeof(short);  // bytes
  else
    zoneSize = RT_BUFFER_SIZE*sizeof(short);  // bytes
...
int RTSoundIO :: playBuffer(short *buf, int bufsize)
{
  ...
  // This section of code implements method two above.

  /*
   * Microsloth says that the safePos is about 15 ms ahead of
   * playPos.  If you really need to reduce delay, you can try
   * moving your "safePos" closer to the play pointer.  You'll
   * be treading on dangerous ground, but then again, you're
   * obviously using Windoze so you're already familiar with
   * such uncertainty!  I've been able to lop off 2-3 ms in
   * some circumstances.
   */
  //DWORD backup = (DWORD) (0.003 * SRATE * sizeof(short));
  //safePos = (safePos + m_cbBufSize - backup) % m_cbBufSize;

  // Assume that the write pointer (m_cbBufOfffset) is always
  // in front of safePos.  If necessary, add an offset.
  DWORD offset = 0;
  if( safePos > m_cbBufOffset ) offset = m_cbBufSize;

  DWORD deltaPos = m_cbBufOffset + offset - safePos;
  ...
  while ( deltaPos > zoneSize ) {
     ...
     */
    millis = (DWORD) ((deltaPos * 200.0) / ( sizeof(short) * SRATE));
    ...
    deltaPos = m_cbBufOffset + offset - safePos;
  }
  // End of method two.

  /*
  // This section of code implements method one as mentioned above.
  DWORD endWrite;
  ...
  while ( playPos < endWrite ) {
    ...
    // time = distance * (milliseconds/second) / 
    //        ((bytes/sample) * (samples/second)),
    // rounded up.
...
#endif
// md5sum:	80dd4abd4a6b429b723035eb6be8a727  RTSoundIO.cpp
